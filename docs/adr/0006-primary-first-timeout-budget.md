...
**Rotation and retry are separate quotas.** Timeout never rotates credentials; rotation stays RateLimited/QuotaExhausted only (see the provider credential pool ADR). The rotation quota is the credential pool size; the retry quota is `retry.max_attempts`, counted by retries alone, so rotations no longer consume retry headroom. Backoff stays linear (`multiplier × retry_count`, capped at `retry.max_wait`) with no jitter: at three attempts in a single-user CLI, exponential backoff and jitter buy nothing and burn deadline.
