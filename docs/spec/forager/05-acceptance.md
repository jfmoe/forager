...
2. **HTTP fixture**（本地桩 + 真实 reqwest 栈）：9 provider 响应解码、SSE、MCP 各配 fixture 组。主搜索覆盖 xAI role-array、failed/incomplete 分类、completed 无文本失败、OpenAI-compatible 非空 clean EOF，以及两者遇到畸形非空 JSON 帧都整体 Runtime；normalizer 只覆盖 think block、显式来源 grammar、纯数字引用序号标题按无标题处理、空结果 Runtime 与公开 URL 去重。MCP 覆盖 optional-session、AnySearch 固定 client header、非数组 content Runtime 与 structuredContent；成功协议超 4 MiB 整体失败。
3. **offline-e2e（验收主面）**：spawn 真实 `forager` 二进制 + transport-mock，覆盖路由/归因/fallback/退出码/config/plan/journal 副作用/I0-6/I0-7。CLI 覆盖 JSON 飞行前终态与 Markdown/content 豁免；删除 validation 后未知参数/配置；`extra_sources` 的 `0 → web 3/docs 1/vertical 1`、20 成功与 21 clap 失败零网络；shallow doctor 聚合；debug/trace attempts 投影；journal 只清理自有独占文件；Recovery Manifest 成功/写失败 locator。Search Candidate 覆盖统一信封、Context7 typed locator 与无 URL evidence、Exa 字段投影、AnySearch Markdown/required 标记；Supplemental 覆盖 Tavily advanced profile、合法空集 fallback、严格 container 与窄 URL 容错。Web Fetch 覆盖三家 request profile、共享 provider 顺序、薄正文 Quality fallback、全链 Quality和 4 MiB 截断；Firecrawl fixture 精确断言 Markdown、`onlyMainContent: true`、`timeout: 60000`、PDF `pageMarkers` parser 与 `waitFor` 缺失，并复用薄正文用例证明每个逻辑 attempt 只 scrape 一次。map 覆盖 timeout 10/150、depth 1/5、breadth 1/500 的 wire，以及越界 clap 退出 2 与零网络，单次 attempt cap 使用合法命令 timeout + 较小 provider timeout。direct fetch、search-side Web Fetch 与 research 共享 engine 入口，不复制调用面矩阵；不新增 live-e2e。平台命令覆盖参数表中每个参数「输入 → 预期请求 query」的 wire 编码、首页/中间页/末页/空页的 `next_cursor` 与 cursor 恢复原请求、Atom error entry 退 4、合法空 feed 退 0、飞行前退 2/3 的每种情形且零网络、匿名 route 的 `credential_index` 为 0，以及限速窗口超出 Deadline 时 Timeout 且不发送；doctor 深探与浅检可达性都经过限速。SSRN 另覆盖：Crossref 前缀检索的 query、`rows`、`offset`、`sort`、`order`、`select`；按过滤前条数判断末页；分页上限处不签发 cursor；跳过非 SSRN DOI 并写诊断、保留 `journal-article`；按 ref、摘要页 URL、短 URL、DOI 与 doi.org URL fetch 同一 DOI；`metadata` 与 `abstract` 深度及缺摘要时 Quality 退 5；404 为带 Crossref 消息的 Parameter、429 为 RateLimited、DOI 不一致与 `message-type` 不符为 Runtime。`ssrn_browser` 以测试生成的假 `opencli` 可执行文件（记录 argv，按预设输出外壳与退出码）覆盖：argv 与会话 flag、外壳解码与契约版本不符、`no_results` 与 EMPTY_RESULT 的区别、退出码映射（含 `ADAPTER_LOAD` 的安装提示）、工作截止点强杀整个进程组且不留进程、结果页与详情页 id 核对、Crossref 缺摘要时落到浏览器、`full_text` 跳过浏览器、默认 order 不运行浏览器，以及 doctor 只在 order 启用时运行 `contract` 命令；这些测试不声称证明 JS adapter 的行为，JS 只由 live smoke C22/C23 验证。平台 fixture 与 smoke 的测试配置把未用到的平台 order 置空，或把其 route 指向 fixture，普通测试不访问外网。
4. **live-e2e**：见下。

## live-e2e（`smoke --live` 用例面）

- 离线档（无 `--live`）：不碰网络——配置可解析且过值域、registry 完整、凭据在位状态（不验真伪）、journal 目录可写；进 PR CI。
- `--live` 用例以子进程执行，子进程固定 `FORAGER_JOURNAL__ENABLED=false`：金丝雀调用不写入用户的 Search Result Journal，避免污染复盘数据。
- `--live` 用例面分两类，全部原子化编 ID（H12 修订：原 L0–L8 粗分档废止，改为下述规范化清单；「全绿」以用例 ID 计）：

**流程门 L0**：`doctor --provider <p>` × 9（L0.1–L0.9，每 provider 一发深探）——凭据迁移验收门，每里程碑前置。
...
| C20 | ssrn_crossref | platform ssrn：search（`platform ssrn search` 直连） | HTTP |
| C21 | ssrn_crossref | platform ssrn：fetch（`platform ssrn fetch ssrn:2042750 --depth metadata` 直连） | HTTP |
| C22 | ssrn_browser | platform ssrn：search（按 order 门控：`platforms.ssrn.order` 含 `ssrn_browser` 时才运行；需要真实 OpenCLI、Chrome 与已安装的 adapter） | process |
| C23 | ssrn_browser | platform ssrn：fetch（`platform ssrn fetch ssrn:2042750 --depth metadata`，按 order 门控） | process |
...
- 档次落流程不落旗标：每里程碑跑 L0 全绿 + 手动 P1/P2/C14；切换前跑 `smoke --live` 全量（P1–P2 + C01–C23）；live 档不进 PR CI。
