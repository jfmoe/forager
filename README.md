...
`fetch` 对 URL 与 PDF 使用同一条默认 `Firecrawl → Tavily → Jina` 链，成功时只返回 provider 无关的 Markdown 正文；诊断与 provider attempts 不会混入正文。
...
Skill 要求 `forager >= 0.5.2`。外部 Agent 的上下文保护方式见
